...
const alarmTitleMap: Record<string, string> = {
...
const normalizeAlarmTitle = (rawTitle?: string) => {
  const raw = String(rawTitle || '').trim();
  if (!raw) return '未知告警';
  if (alarmTitleMap[raw]) return alarmTitleMap[raw];

  const upperRaw = raw.toUpperCase();
  if (alarmTitleMap[upperRaw]) return alarmTitleMap[upperRaw];
  if (upperRaw.startsWith('VIDEO_DEVICE_')) return '视频设备告警';

  return raw;
};

const normalizeAlarmEntry = (item: any): AlarmRecord => {
  const normalizedType = String(item?.type || '').toLowerCase() === 'fence' ? 'fence' : 'video';
  const normalizedStatus = String(item?.status || '').toLowerCase();
  const normalizedLevel = String(item?.level || '').toLowerCase();

  return {
    id: String(item?.id || `tmp-${Date.now()}`),
    type: normalizedType,
    title: normalizeAlarmTitle(item?.title),
    description: String(item?.description || ''),
    time: String(item?.time || new Date().toISOString()),
    level: (normalizedLevel === 'high' || normalizedLevel === 'medium' || normalizedLevel === 'low')
      ? normalizedLevel
      : 'medium',
    status: (normalizedStatus === 'pending' || normalizedStatus === 'resolved' || normalizedStatus === 'ignored')
      ? normalizedStatus
      : 'pending',
    location: String(item?.location || '未知位置'),
    deviceName: String(item?.deviceName || '未知设备'),
...
useEffect(() => {
  ...
  if (storedAlarms) {
    ...
    const normalizedParsed = Array.isArray(parsed) ? parsed.map(normalizeAlarmEntry) : [];
    const allAlarms = [...normalizedParsed, ...mockFenceAlarms, ...mockVideoAlarms].sort(
...
useEffect(() => {
  const handleAlarmAdded = (event: CustomEvent) => {
    setAlarms(prev => [normalizeAlarmEntry(event.detail), ...prev]);
  };
