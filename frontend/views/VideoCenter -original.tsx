...
export default function VideoCenter() {
  ...
  // 过滤后的设备列表（用于网格显示）
  const filteredDevicesForGrid = devices.filter(device => {
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="w-28">
            ...
            >
              {companies.map(company => (
                <option key={company} value={company}>{company === 'all' ? '公司' : company}</option>
              ))}
            </select>
          </div>
          ...
          <div className="w-28">
            ...
            >
              {projects.map(project => (
                <option key={project} value={project}>{project === 'all' ? '项目' : project}</option>
              ))}
            </select>
          </div>
          ...
          <div className="relative">
            <button
              ...
              className="px-3 py-1.5 text-sm bg-cyan-500/20 text-cyan-300 rounded-md hover:bg-cyan-500/30 flex items-center gap-2"
            >
              <Camera size={14} />
            ...
            {showDeviceSelector && (
              <>
                <div className="fixed inset-0 z-10" onClick={() => setShowDeviceSelector(false)} />
                <div className="absolute top-full left-0 mt-1 w-64 bg-slate-800 border border-slate-700 rounded-md shadow-xl z-20 max-h-64 overflow-y-auto p-2">
      ...
      <div className="flex-1 overflow-hidden flex flex-col h-full">
        <div className="flex justify-between items-center mb-3 flex-shrink-0">
          <h3 className="text-lg font-bold text-cyan-300">
            监控设备 
            <span className="text-sm text-slate-400 ml-2">({filteredDevicesForGrid.length}个设备)</span>
          </h3>
          <div className="flex gap-2">
            <button onClick={() => setGridCols(2)} className={`px-2 py-1 text-xs rounded ${gridCols === 2 ? 'bg-cyan-500/30 text-cyan-300' : 'bg-slate-700'}`}>2列</button>
            <button onClick={() => setGridCols(3)} className={`px-2 py-1 text-xs rounded ${gridCols === 3 ? 'bg-cyan-500/30 text-cyan-300' : 'bg-slate-700'}`}>3列</button>
            <button onClick={() => setGridCols(4)} className={`px-2 py-1 text-xs rounded ${gridCols === 4 ? 'bg-cyan-500/30 text-cyan-300' : 'bg-slate-700'}`}>4列</button>
          </div>
        </div>
        <div className={`grid gap-4 flex-1 overflow-auto`} style={{ gridTemplateColumns: `repeat(${gridCols}, minmax(0, 1fr))` }}>
          {filteredDevicesForGrid.map((device) => (
            <DeviceCard
              key={device.id}
              device={device}
              previewStream={previewStreams[device.id]}
              isLoading={previewLoading[device.id]}
              error={previewErrors[device.id]}
              onLoadPreview={() => loadPreviewStream(device)}
              onPlay={() => {
      ...
      {maximizedVideo && (
        <div className="fixed inset-0 z-[200] bg-[radial-gradient(circle_at_15%_8%,rgba(34,211,238,.15),transparent_35%),linear-gradient(140deg,#020617,#0b1f3f_50%,#102a5e)] flex flex-col p-4 gap-4">
          <div className="flex justify-between items-center">
            ...
            <button onClick={() => { setMaximizedVideo(null); setStreamUrl(null); }} className="p-2 text-slate-400 hover:bg-rose-500/20 hover:text-rose-300 rounded-full transition-colors"><X size={24} /></button>
          </div>
          <div className="flex-1 flex gap-4 min-h-0">
            <div className="flex-1 flex flex-col bg-slate-900/65 rounded-lg border border-blue-300/25 overflow-hidden">
              {streamUrl ? (
                ...
                <div className="flex items-center justify-center flex-1"><Loader className="animate-spin text-cyan-300" size={48} /></div>
              )}
            </div>
            {streamUrl && (
              <div className="w-80 flex flex-col gap-3 h-full">
                <div className="bg-slate-900/75 rounded-lg border border-blue-300/25 p-4 shadow-lg shrink-0">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="font-bold text-slate-100 flex items-center gap-2"><Shield size={18} className="text-cyan-300" /> AI 智脑控制</h3>
                    <div className="flex gap-2">
                      <button onClick={() => handleToggleAll(true)} className="text-[10px] font-bold text-cyan-200 bg-cyan-500/20 border border-cyan-300/30 px-2 py-1 rounded hover:bg-cyan-500/30 transition-colors">全开启</button>
                      <button onClick={() => handleToggleAll(false)} className="text-[10px] font-bold text-rose-200 bg-rose-500/20 border border-rose-300/30 px-2 py-1 rounded hover:bg-rose-500/30 transition-colors">全关闭</button>
                    </div>
                  </div>
                  <div className="relative mb-4">
                    <select className="w-full p-2.5 bg-slate-950/65 border border-blue-300/25 rounded-md text-sm text-slate-200 outline-none focus:ring-2 focus:ring-cyan-400/40 appearance-none cursor-pointer" value="" onChange={(e) => handleSingleAI(e.target.value)} disabled={aiLoading}>
                      <option value="" disabled>＋ 添加/切换算法功能</option>
                      {algos.map(algo => (<option key={algo.id} value={algo.id}>{activeAlgos.includes(algo.id) ? "✅ 已开启：" : "⭕ 未开启："} {algo.name}</option>))}
                    </select>
                    <div className="absolute right-3 top-3.5 pointer-events-none text-slate-500"><Settings size={14} /></div>
                  </div>
                  <div className="flex flex-wrap gap-2 min-h-[24px]">
                    {activeAlgos.length === 0 ? (<span className="text-[11px] text-slate-400 italic flex items-center gap-1"><AlertCircle size={12} /> 暂无监测任务运行</span>) : (activeAlgos.map(id => { const algo = algos.find(a => a.id === id); return (<div key={id} className="flex items-center gap-1.5 bg-emerald-500/15 border border-emerald-300/35 text-emerald-200 px-2 py-1 rounded text-[11px] animate-pulse"><ShieldCheck size={12} />{algo?.name}</div>); }))}
                  </div>
                  {aiLoading && (<div className="mt-3 pt-3 border-t border-blue-300/20 flex items-center justify-center gap-2 text-[11px] text-cyan-300"><Loader size={12} className="animate-spin" /> 正在同步云端算法状态...</div>)}
                </div>
                <div className="bg-slate-900/75 rounded-lg border border-blue-300/25 overflow-y-auto shadow-lg flex-1 vc-scrollbar">
                  <PTZControlPanel video={maximizedVideo} onSuccess={(msg) => console.log(msg)} onError={(err) => console.error(err)} />
                </div>
                <div className="bg-slate-900/75 rounded-lg border border-blue-300/25 p-4 shadow-lg shrink-0">
