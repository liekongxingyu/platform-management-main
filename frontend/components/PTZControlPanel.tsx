...
useEffect(() => {
  const handleKeyDown = (e: KeyboardEvent) => {
    if (!canPTZ || document.activeElement?.tagName === 'INPUT') return;
    switch (e.key) {
      case 'ArrowUp': e.preventDefault(); startMove('up'); break;
      case 'ArrowDown': e.preventDefault(); startMove('down'); break;
      case 'ArrowLeft': e.preventDefault(); startMove('left'); break;
      case 'ArrowRight': e.preventDefault(); startMove('right'); break;
      case '+': case '=': e.preventDefault(); startMove('zoom_in'); break;
      case '-': e.preventDefault(); startMove('zoom_out'); break;
    }
  };
  ...
  const handleKeyUp = (e: KeyboardEvent) => {
    if (!canPTZ) return;
    switch (e.key) {
      case 'ArrowUp': stopMove(); break;
      case 'ArrowDown': stopMove(); break;
      case 'ArrowLeft': stopMove(); break;
      case 'ArrowRight': stopMove(); break;
      case '+': case '=': stopMove(); break;
      case '-': stopMove(); break;
    }
  };
...
}, [canPTZ, video.id, startMove, stopMove]);
...
const handleCreatePreset = async () => {
  ...
  return (
    <div className="bg-slate-950/70 rounded-lg shadow-md p-4 select-none text-slate-100 border border-blue-300/25">
      <h3 className="text-lg font-semibold mb-2 text-cyan-200">云台控制</h3>
      <p className="text-xs text-slate-400 mb-4">
        ⌨️ 键盘快捷键: 方向键 ↑↓←→ 控制方向 | +/- 控制变焦
      </p>
